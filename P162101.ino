...
void loop() {
  ...
  switch(display) {
    ...
    default:
      ...
      //snprintf(str_buffer, sizeof(str_buffer), "%u", display);
      //snprintf(str_buffer, sizeof(str_buffer), "%u", temp);
      //snprintf(str_buffer, sizeof(str_buffer), "%d", g_rtcPulse);
  ...
  /*if (new_display != display) {
    ...
    int transition_shift = transition_frame;
    for (int i = 0; i < MAT_COLS; i++) {
      if (init_sequence == SEQ::DISPT) {
        backbuffer[i] <<= 16 - transition_shift;
      } else {
        backbuffer[i] >>= transition_shift;
      }
    }
    switch (init_sequence) {
      case SEQ::DISPT:
        if (transition_shift >= 16) {
        ...
        break;
      case SEQ::IDLE:
        if (transition_shift >= 8) {
          xchg(display, new_display);
          init_sequence = SEQ::DISPT;
        }
        break;
    }
    ++transition_frame;
